...
if CLIENT then
    ...
    local function createBall(x, y, rarity)
        TableInsert(gatchaBalls, {x=x, y=y, rarity=rarity, vx=MathRand(-5,5), vy=MathRand(-5,5), a=MathRand(0, 2 * MathPi)})
    end
    ...
    local function removeBall()
        ...
        for i, ball in ipairs(gatchaBalls) do
        ...
        TableRemove(gatchaBalls, closestIndex)
    end
    ...
    concommand.Add("startgatcha", animateGacha)
    ...
    AddHook("HUDPaint", "Gamer_HUDPaint", function()
        ...
        for _, ball in ipairs(gatchaBalls) do
        ...
        for i = 1, #gatchaBalls do
            for j = i + 1, #gatchaBalls do
                local ball1 = gatchaBalls[i]
                local ball2 = gatchaBalls[j]
